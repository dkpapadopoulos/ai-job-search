...
% NOTE: Inside an itemize, write "\item {[text]}", never "\item [text]".
% A "[" immediately after \item is parsed as \item's optional label
% argument, so the placeholder becomes a left-margin label and overflows
% off the left edge of the page. Brace-wrapping the placeholder protects it.
% (This is also why \cventry is called directly here, not wrapped in an
% outer itemize: it lays out its own columns and needs no list wrapper.)
